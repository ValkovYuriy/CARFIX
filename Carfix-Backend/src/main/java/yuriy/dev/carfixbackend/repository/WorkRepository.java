...
public interface WorkRepository extends JpaRepository<Work, UUID> {
    ...
    @Override
    @Query("select new Work(w.id,w.name,w.description,wp.price,w.image) from Work w join WorkPrice wp on w.id = wp.work.id " +
    ...
    @Query("SELECT new Work(w.id,w.name,w.description,wp.price,w.image)" +
            "FROM Work w " +
    ...
    @Query("SELECT new yuriy.dev.carfixbackend.dto.PopularWork(w.name,w.image, COUNT(*)) " +
            "FROM Order o JOIN o.works w " +
            "WHERE EXTRACT(YEAR FROM o.orderDate) = EXTRACT(YEAR FROM CURRENT_DATE) " +
            "GROUP BY w.name,w.image " +
            "ORDER BY COUNT(*) DESC " +
            "LIMIT 5")
